"""Train a binary classifier using SimCLR."""
...
def main(args: argparse.Namespace) -> None:
    """Main function to train with SimCLR.
    ...
    validation_directory = (
        "0_data/binary/validation"  # "0_data/group_safe_binary/validation"
    )
    all_training = (
        "0_data/binary/all_training"  # "0_data/group_safe_binary/all_training"
    )
    test_directory = "0_data/binary/test"  # "0_data/group_safe_binary/test"

    # best_model = wandb.restore('simclr_model.weights.h5',
    # run_path="freja-thoresen/SimCLR/lrcnvnrs")

    # Change if a users wants to train a different network
    network = SimCLR(
        path="2_contrastive_learning/models/pretrained_r50_1x_sk0/",
    ...
    if args.c:
        ...
        split_dataset_into_test_and_train_sets(
            args.source, all_training, test_directory, 0.15
        )
        network.experiment1(args)
    else:
        # Run and evaluate model on validation set
        split_dataset_into_test_and_train_sets(
            args.source, all_training, test_directory, 0.2
        )
        network.preprocessing(all_training)
        # network.create_train_val_split_with_group_limit(training_directory,
        # validation_directory, training_directory, validation_directory)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Trains binary classifier on provided dataset"
    )
    parser.add_argument(
        "source",
        type=str,
    ...
    parser.add_argument(
        "-c", action="store_true", help="Enables cross validation training"
    )
    parser.add_argument(
        "-f", action="store_true", help="Enables fine tuning after training."
    )
    parser.add_argument(
        "-x",
        action="store_true",
        help="Enables experiment 1 - Check for repeated false positives",
    )
    parser.add_argument(
        "-t", action="store_true", help="Runs only 2 epochs - for testing purposes"
    )
    parser.add_argument(
        "-g",
        action="store_true",
    ...
    parser.add_argument(
        "-T", action="store_true", help="Calculate final testing of classifier"
    )
    args = parser.parse_args(
        [
            "/home/spaceship/users/freja_thoresen/thin-slice-classifier/0_data/datasets/rock_type"
        ]
    )
    main(args)  # '-c', '-t', -f', '-f',
