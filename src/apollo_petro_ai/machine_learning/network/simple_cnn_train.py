...
def train(start: str, run_id: int) -> tuple:
    ...
    callback = callbacks.ModelCheckpoint(
        "output/binary-model_{epoch:02d}-{val_loss:.2f}-{val_accuracy:.2f}.h5",
        save_best_only=True,
    )
    ...
    # Since early stopping will restore weights to best performing model,
    # save the accuracy of that model
...
def cross_valid_train(path: str, n_splits: int) -> None:
...
def single_run(path: str) -> None:
    ...
    """
    split_dataset_into_test_and_train_sets(
        path, training_directory, test_directory, 0.25
    )
    start = time.strftime("%Y%m%d-%H%M%S")
