        ...
        if label_file.endswith(".lbl"):
            ...
            with open(path, "r") as file_stream:
                ...
                if sample is None:
                    ...
                    descriptor, value = parse_multiline_description(
                        cur_line, file_stream
                    )
                    sample[descriptor] = value
                ...
                photo["PATHS"] = list(
                    filter(lambda x: photo["FILENAME"] in x.lower(), img_list)
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Processing sample metadata from LBL files into one data store"
    )
    parser.add_argument(
        "directory", action="store", help="directory of LBL files to be parsed"
    )
    parser.add_argument("output", type=str, action="store", help="output file name")
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="outputs processed data"
    )
