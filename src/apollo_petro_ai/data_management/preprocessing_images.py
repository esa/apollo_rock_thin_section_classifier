...
def RGB_TO_HSI(img):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        hsi = cv2.merge((calc_hue(red, blue, green),
...
# OUTPUT: dictionary
def resize_and_save_into_dict(data_dir, dir_output, img_rows, img_cols, median_filter, histogram_equalization):
    dict_photos = {}
    for folder in os.listdir(data_dir):
        ...
        folder_resize_hsi = dir_output + "/" + folder[0:folder.find("_")] + "_HSI_resize/"
        ...
        for photo in list_photos:
            ...
            image_resize_rgb = resize(img, (img_cols, img_rows), anti_aliasing=True)*255
            image_resize_hsi = RGB_TO_HSI(image_resize_rgb)
            Image.fromarray(image_resize_hsi.astype(np.uint8)).save(photo_path_hsi_resize)
            Image.fromarray(image_resize_rgb.astype(np.uint8)).save(photo_path_rgb_resize)
            ...
            image_resize_hsi_normalized = np.dstack((h_normalized, s_normalized, i_normalized))
...
def create_features_list(dict_photos, feature_all, dir_output):
    ...
    for photo_plane in dict_photos[list(key_list)[0]]: 
        ...
        if photo_plane["filename_rgb"][0:photo_plane["filename_rgb"].rfind("_")] != \
            ...
            if feature_all:
                array_features = np.concatenate((array_rgb_plane, array_hsi_plane, array_rgb_cross, array_hsi_cross),
                                                axis=2)
            else:
                non_linear_combi_rgb = np.sqrt(np.square(array_rgb_plane) + np.square(array_rgb_cross))
                non_linear_combi_hsi = np.sqrt(np.square(array_hsi_plane) + np.square(array_hsi_cross))
                array_features = np.concatenate((non_linear_combi_rgb, non_linear_combi_hsi), axis=2)
                photo_path_rgb = folder_non_lin_rgb + "/" + photo_plane['filename_rgb']
                photo_path_hsi = folder_non_lin_hsi + "/" + photo_plane['filename_hsi']
                Image.fromarray((non_linear_combi_rgb * 255).astype(np.uint8)).save(photo_path_rgb)
                Image.fromarray((non_linear_combi_hsi * 255).astype(np.uint8)).save(photo_path_hsi)
            photo = {"filename_rgb": photo_plane["filename_rgb"], "features": array_features}
...
# OUTPUT: list of dictionaries with photos and associated features
def pre_processing(data_dir, dir_output, feature_all, median_filter, histogram_equalization):
    img_rows, img_cols = 120, 89 
    clean_folder(data_dir)
    dict_photos = resize_and_save_into_dict(data_dir, dir_output, img_rows, img_cols,
