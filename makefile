...
install: ## Install dependencies
	@echo "Installing the 'apollo_petro_ai' project..."
	...
	@echo "Installed the 'apollo_petro_ai' project! You can now activate your virtual environment with 'source .venv/bin/activate'."
...
add-repo-to-git:
	...
	@if [ "$(shell git remote)" = "" ]; then \
		git remote add origin git@github.com:esa/apollo_petro_ai.git; \
	fi
...
publish-docs:  ## Publish documentation to GitHub Pages
	...
	@echo "Updated documentation website: https://esa.github.io/apollo_petro_ai"

test:  ## Run tests
	@uv run pytest && uv run readme-cov

docker:  ## Build Docker image and run container
	@docker build -t apollo_petro_ai .
	@docker run -it --rm apollo_petro_ai
